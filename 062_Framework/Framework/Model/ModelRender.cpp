...
void ModelRender::LateUpdate()
{
	if (updateTransforms.size() < 1)
		return;


	for (UpdateTransformDesc& desc : updateTransforms)
		memcpy(&worlds[desc.index], &transforms[desc.index]->World(), sizeof(D3DMATRIX));
...
{
	UpdateTransformDesc desc;
	desc.index = index;
	desc.Transform = transforms[index];

	updateTransforms.push_back(desc);
