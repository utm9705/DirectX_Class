...
{
	shadowDepthBuffer = new ConstantBuffer(&shadowDepthDesc, sizeof(ShadowDepthDesc));
	sShadowDepthBuffer = shader->AsConstantBuffer("CB_ShadowDepth");

	shadowBuffer = new ConstantBuffer(&shadowDesc, sizeof(ShadowDesc));
	...
	depthRender->GetTransform()->Scale(200, 200, 1);
	depthRender->GetTransform()->Position(100, 100, 0);
...
void Shadow::Update()
{
	...
	ImGui::SliderFloat("Shadow Bias", &shadowDesc.Bias, 0.0001f, 0.1f, "%f");
