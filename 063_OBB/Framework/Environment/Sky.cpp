	...
	, timeFactor(0.25f)
{
	...
	targetBuffer = new ConstantBuffer(&targetDesc, sizeof(TargetDesc));
	sTargetBuffer = shader->AsConstantBuffer("CB_Target");

	nightBuffer = new ConstantBuffer(&nightDesc, sizeof(NightDesc));
	...
	sCloudMap = shader->AsSRV("CloudMap");
	sMoonMap = shader->AsSRV("MoonMap");
}
...
void Sky::Render()
{
	...
	//SkyDome
	{
		...
		sRayleighMap->SetResource(scattering->Rayleigh()->SRV());
		sMieMap->SetResource(scattering->Mie()->SRV());
